...
public class ProductService : BaseService<Product>, IProductService
{
    ...
    public Task<IEnumerable<Product>> GetByName(string name) => 
        Task<IEnumerable<Product>>.Factory.StartNew(() => Repository.GetAll().Where(x => x.Name == name));

    public Task<Product> GetById(string token, int id) =>
        Task<Product>.Factory.StartNew(() =>
        {
    ...
    public Task<bool> ChangeCategory(string token, Category category, Product product) =>
        Task<bool>.Factory.StartNew(() => category is not null && ChangeProperty(token, x => x.Category = category, product));
    ...
    public Task<bool> Remove(string token, Product entity)=>
        Task<bool>.Factory.StartNew(() => entity != null && Remove(token, entity.Id).Result);
