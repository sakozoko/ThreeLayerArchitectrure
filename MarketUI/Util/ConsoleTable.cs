...
public class ConsoleTable
{
    ...
    public ConsoleTable() : this(new ConsoleTableOptions())
    {
    }

    public ConsoleTable(ConsoleTableOptions consoleTableOptions) : this(new List<int>(), consoleTableOptions)
    {
        
    }

    public ConsoleTable(IEnumerable<int> paddings) : this(paddings,new ConsoleTableOptions())
    {

    }

    public ConsoleTable(IEnumerable<int> paddings, ConsoleTableOptions consoleTableOptions)
    {
    ...
    public ConsoleTable AddCustomFormat(Type targetType, string format)
    {
        if (targetType is null || string.IsNullOrWhiteSpace(format))
            throw new ArgumentNullException(targetType is null ? nameof(targetType) : nameof(format), "Value is null");
    ...
    public ConsoleTable AddCustomFormat(int indexOfTableColumn, string format)
    {
    ...
    public ConsoleTable AddAlignment(Alignment alignment)
    {
    ...
    public ConsoleTable AddAlignment(Alignment alignment, int indexOfColumn)
    {
    ...
    public ConsoleTable AddSeparatorForEachRow()
    {
        _consoleTableOptions.SeparateEachRow = true;
        
        return this;
    }
    public ConsoleTable AddSeparatorForEachRow(char separator)
    {
        _consoleTableOptions.SeparateEachRow = true;
    ...
    private string GetHeader(string format)
    {
        return string.Format(format, Columns.Select((c, i) => _consoleTableOptions.Alignments[i] == Alignment.Center
                ? new string(' ', (CurrentPadding[i] - c.Length) / 2) + c +
                  new string(' ', CurrentPadding[i] - c.Length - (CurrentPadding[i] - c.Length) / 2):c)
            .ToArray<object>());
    }
        ...
        return ModifiedRows.Select(row =>
            string.Format(format, row.Select((c, i) =>_consoleTableOptions.Alignments[i]==Alignment.Center?
                ...
                new string(' ',
                    ...
                    (CurrentPadding[i] - c.ToString().Length) / 2):c
            ).ToArray())).ToList();
    }
        ...
        ModifiedRows = Rows.Select(x =>
            x.Select((v, i) =>
                _consoleTableOptions.CustomFormats.ContainsKey(i)
                    ? string.Format($"{_consoleTableOptions.CustomFormats[i]}",v) : v).
                Select(v=>
                _consoleTableOptions.CustomFormats.ContainsKey(v.GetType())
                    ? string.Format($"{_consoleTableOptions.CustomFormats[v.GetType()]}",v) : v).
                ToArray()).
            ToList();
    }
